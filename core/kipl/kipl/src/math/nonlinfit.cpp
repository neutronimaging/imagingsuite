...
void LevenbergMarquardt::mrqcof(Nonlinear::TNTfit::FitFunctionBase &fn, Array1D<double> &x, Array1D<double> &y,
...
void LevenbergMarquardt::covsrt(Array2D<double> &covar, Nonlinear::TNTfit::FitFunctionBase &fn)
{
//Expand in storage the covariance matrix covar, so as to take into account parameters that
//are being held fixed. (For the latter, return zero covariances.)
    int i,j,k;
