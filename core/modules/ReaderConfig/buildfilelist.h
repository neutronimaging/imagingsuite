...
std::map<float,std::string> READERCONFIGSHARED_EXPORT BuildProjectionFileList(std::list<FileSet> &il, int sequence, double arc);
std::map<float,std::string> READERCONFIGSHARED_EXPORT BuildProjectionFileList(std::list<FileSet> &il, std::list<int> &skiplist, int sequence, double arc);
