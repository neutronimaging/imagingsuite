...
# `$CPCMD $REPOSPATH/install/lib/libNeXus.1.0.0.dylib    $DEST/Contents/Frameworks`
# `$CPCMD $REPOSPATH/install/lib/libNeXusCPP.1.0.0.dylib $DEST/Contents/Frameworks`
# `$CPCMD $REPOSPATH/install/lib/libhdf5*.dylib          $DEST/Contents/Frameworks`
...
`$CPCMD /usr/local/lib/libNeXus*.dylib                 $DEST/Contents/Frameworks`
...
cd $QTBINPATH
echo "Do deploy..."
./macdeployqt $DEST -libpath=$REPOSPATH/install/lib -always-overwrite #-dmg
...
# install_name_tool -change libNeXus.1.dylib @rpath/libNeXus.1.dylib muhrectomo.cpython-39-darwin.so
# install_name_tool -change libNeXusCPP.1.dylib @rpath/libNeXusCPP.1.dylib muhrectomo.cpython-39-darwin.so
