...
int IMAGINGMODULESSHARED_EXPORT BBLogNorm::Configure(KiplProcessConfig config, std::map<std::string, std::string> parameters)
{
    ...
    externalmaskname=GetStringParameter(parameters,"externamaskname");
    ...
    bUseExternalMask = kipl::strings::string2bool(GetStringParameter(parameters,"bUseExternalMask"));
    ...
    dose_roi = nOriginalNormRegion;

    if ( m_Config.mImageInformation.bUseROI)
    {
        msg<<"using image roi with coordinates: "
          <<  m_Config.mImageInformation.nROI[0] << " "
          <<  m_Config.mImageInformation.nROI[1] << " "
          <<  m_Config.mImageInformation.nROI[2] << " "
          <<  m_Config.mImageInformation.nROI[3] << " ";
        logger(kipl::logging::Logger::LogDebug,msg.str());
    }
    else
    {
        msg<<"not using image roi";
        logger(kipl::logging::Logger::LogDebug,msg.str());
    }

    if (bUseExternalMask)
    {
        msg<<"using external BB mask";
        logger(kipl::logging::Logger::LogDebug,msg.str());
        ImageReader reader;
        if (m_Config.mImageInformation.bUseROI)
        {
            mMaskBB = reader.Read(externalmaskname,
    ...
    if (roi_bb_x>0 && roi_bb_y>0 && !bUseExternalMask) {}
    else {
...
//    memcpy(nNormRegion,nOriginalNormRegion,4*sizeof(size_t)); // seems not used
...
//    else
//    {
//        LoadReferenceImages(nullptr); // TODO correct for valid input
//    }
    return true;
}
...
std::map<std::string, std::string> IMAGINGMODULESSHARED_EXPORT   BBLogNorm::GetParameters() {
    ...
    parameters["bUseExternalMask"] = kipl::strings::bool2string(bUseExternalMask);
    parameters["externalmaskname"] = externalmaskname;
...
void IMAGINGMODULESSHARED_EXPORT  BBLogNorm::LoadReferenceImages(const std::vector<size_t> &roi)
{
    ...
    if (!roi.empty())
    {
        ...
        msg << "Loading reference images without roi";
        logger(kipl::logging::Logger::LogDebug,msg.str());
    }
...
void IMAGINGMODULESSHARED_EXPORT  BBLogNorm::LoadExternalBBData(const std::vector<size_t> &roi){
    ...
    if (blackbodysampleexternalname.empty() || nBBextCount==0)
        throw ImagingException("The pre-ed sample with BB image mask is empty", __FILE__, __LINE__);
...
void IMAGINGMODULESSHARED_EXPORT  BBLogNorm::PrepareBBData(){
    ...
    logger(kipl::logging::Logger::LogDebug,"PrepareBBData begins...");
...
//    int diffroi[4] = {int(BBroi[0]), int(BBroi[1]), int(BBroi[2]), int(BBroi[3])};
    ...
    logger(kipl::logging::Logger::LogDebug,"done.");
...
void IMAGINGMODULESSHARED_EXPORT  BBLogNorm::PreparePolynomialInterpolationParameters()
{
    ...
    if (!bUseExternalMask)
    {
        ...
        try {
            ...
            msg.str(""); msg<<"Failed to compute bb_ob_parameters with external BB mask. Please check your inputs." << e.what();
            logger(kipl::logging::Logger::LogDebug,msg.str());
            throw ModuleException("Failed to compute bb_ob_parameters with external BB mask. Please check your inputs.", __FILE__,__LINE__);
        }

        catch(ImagingException &e){
            msg.str(""); msg<<"Failed to compute bb_ob_parameters with external BB mask. Please check your inputs." << e.what();
            logger(kipl::logging::Logger::LogDebug,msg.str());
            throw ImagingException("Failed to compute bb_ob_parameters with external BB mask. Please check your inputs.", __FILE__,__LINE__);
        }

        catch(kipl::base::KiplException &e){
            msg.str(""); msg<<"Failed to compute bb_ob_parameters with external BB mask. Please check your inputs." << e.what();
            logger(kipl::logging::Logger::LogDebug,msg.str());
            throw kipl::base::KiplException("Failed to compute bb_ob_parameters with external BB mask. Please check your inputs.", __FILE__,__LINE__);
        }
    }
...
// here Exceptions need to be added to veirfy if the selected module is compatible with the number of loaded images
     ...
     case (ImagingAlgorithms::ReferenceImageCorrection::Average): {
                 ...
                 sample = BBLoader(
                             m_Config.mImageInformation.sSourceFileMask,
                             m_Config.mImageInformation.nFirstFileIndex +index,
                             1,
                             1.0f,
                             fdarkBBdose,
                             dosesample); // load the projection with angle closest to the first BB sample data
...
int IMAGINGMODULESSHARED_EXPORT  BBLogNorm::PrepareSplinesInterpolationParameters() {
    ...
    float *bb_ob_param = new float[100];
    float *bb_sample_parameters;
    ...
    {
        try {
            bb_ob_param = m_corrector.PrepareBlackBodyImagewithSplinesAndMask(dark, bb, mMaskBB, values);
        }
        catch (ModuleException &e) {
            msg.str(""); msg<<"Failed to compute bb_ob_parameters using externally provided mask. Please check your inputs. " << e.what();
            logger(kipl::logging::Logger::LogDebug,msg.str());
            throw ModuleException("Failed to compute bb_ob_parameters using externally provided mask. Please check your inputs. ", __FILE__,__LINE__);
        }
        catch(ImagingException &e){
            msg.str(""); msg<<"Failed to compute bb_ob_parameters using externally provided mask. Please check your inputs. " << e.what();
            logger(kipl::logging::Logger::LogDebug,msg.str());
            throw ImagingException("Failed to compute bb_ob_parameters using externally provided mask. Please check your inputs. ", __FILE__,__LINE__);
        }
        catch(kipl::base::KiplException &e){
            msg.str(""); msg<<"Failed to compute bb_ob_parameters using externally provided mask. Please check your inputs. " << e.what();
            logger(kipl::logging::Logger::LogDebug,msg.str());
            throw kipl::base::KiplException("Failed to compute bb_ob_parameters using externally provided mask. Please check your inputs. ", __FILE__,__LINE__);
        }
    }
...
int BBLogNorm::ProcessCore(kipl::base::TImage<float,3> & img, std::map<std::string, std::string> & coeff) {
    ...
    if (bUseNormROI==true) {
        ...
        for (int i=0; i<nDose; i++) {
            ...
            float dose = reader.projectionDose(filename,
                                               dose_roi,
                                                  m_Config.mImageInformation.eFlip,
                                                  m_Config.mImageInformation.eRotate,
                                                  1.0f
...
kipl::base::TImage<float,2> BBLogNorm::ReferenceLoader(std::string fname,
                                                      int firstIndex, int N, const std::vector<size_t> &roi,
                                                      float initialDose,
                                                      float doseBias,
...
kipl::base::TImage <float,2> BBLogNorm::BBExternalLoader(std::string fname, const std::vector<size_t> &roi, float &dose)
{
...
kipl::base::TImage <float,3> BBLogNorm::BBExternalLoader(std::string fname, int N, const std::vector<size_t> &roi, int firstIndex,std::vector<float> & doselist){
...
//                size_t dims[]={tempimg.Size(0), tempimg.Size(1),static_cast<size_t>(N)};
            ...
            mylist[i] = bUseNormROI ? reader.projectionDose(filename,
                                                            nOriginalNormRegion,
                                                            m_Config.mImageInformation.eFlip,
                                                            m_Config.mImageInformation.eRotate,
                                                            1.0f
